...
fun countTokens(input: String, diff: Long): Long {
    return input.split("\n\n").map { machine ->
        val (aLine, bLine, priceLine, _) = machine.split("\n")
            .filter { it.isNotEmpty() }

        val a = aLine.replace("Button A: X+", "").replace(" Y+", "").split(",").map { it.toLong() }
            .let { Pair(it[0], it[1]) }
        val b = bLine.replace("Button B: X+", "").replace(" Y+", "").split(",").map { it.toLong() }
            .let { Pair(it[0], it[1]) }
        val prize = priceLine.replace("Prize: X=", "").replace(" Y=", "").split(",").map { it.toLong() + diff }
            .let { Pair(it[0], it[1]) }

        var match = Long.MAX_VALUE

        val initialX = (prize.first / a.first)
        val initialY = (prize.second / a.second)

        val initial = minOf(initialX, initialY) + 1
        var i = initial
        ...
        println("match $match")
        match
    }.filter { it != Long.MAX_VALUE }.sum()
}

// fun countTokens2(input: String): Long {
//     return input.split("\n\n").map { machine ->
//         val (aLine, bLine, priceLine, _) = machine.split("\n")
//             .filter { it.isNotEmpty() }
//
...
//         val b = bLine.replace("Button B: X+", "").replace(" Y+", "").split(",").map { it.toLong() }
//             .let { Pair(it[0], it[1]) }
//         val prize = priceLine.replace("Prize: X=", "").replace(" Y=", "").split(",")
//             .map { it.toLong() + 10000000000000 }
//             .let { Pair(it[0], it[1]) }
//
//         var match = Long.MAX_VALUE
//         var i = 0L
...
//         var last = -1L
//         while (current.first < prize.first && current.second < prize.second) {
//             val remainingX = prize.first - current.first
//             val remainingY = prize.second - current.second
//
//             var step = 1L
...
//                 val two = remainingY / b.second
//
//                  // (1123 + 737x) * 94 + 22y = 10000000008400
//                  // (1123 + 737x) * 34 + 67y = 10000000005400
//
//                 // (674 + 449x) * 26 + 67y = 10000000012748
//                 // (674 + 449x) * 66 + 21y = 10000000012176
//
//                 println("one & two diff ${one - two}") // this is the way - we need to get this closer to zero
//                 if (one == two) {
//                     val currentPrice = 3 * i + one
//                     if (currentPrice < match) {
//                         match = currentPrice
...
//         println(match)
//         match
//     }.filter { it != Long.MAX_VALUE }.reduce { acc, bigInteger -> acc + bigInteger }
// }
...
Button A: X+20, Y+55
Button B: X+59, Y+47
Prize: X=2643, Y=3004""".trimIndent()
